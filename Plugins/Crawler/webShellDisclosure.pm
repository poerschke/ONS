...
sub showResults(){
	...
	$cp =~s/'/\\'/gi;
	$func->insert("INSERT INTO vulnerabilidade(report_id, dados, arq_testados, arq_vuls, var_testadas, var_vuls, reqs, tipo_id) VALUES($report_id, '$cp', 0, 0, 0, 0, 0, 16)");
	$func->insert("UPDATE historico SET crawweb = $qv WHERE report_id= $report_id");
}
